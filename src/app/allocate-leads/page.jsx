...
export default function AllocateLeadsPage() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      const params = Object.fromEntries(
        Object.entries({ ...filters, page, limit: LIMIT }).filter(
          ([_, v]) => v !== "" && v !== null,
        ),
      );
  ...
  useEffect(() => {
    ...
    fetchStats();
    fetchUsers();
  }, [fetchLeads, fetchStats, fetchUsers]);
  ...
  const doAssign = async () => {
    ...
    try {
      if (ids && ids.length > 1) {
        ...
        await api.patch(`/api/leads/${ids[0]}/assign`, {
          assignedTo: assignUser.id,
          assignedToName: assignUser.name,
        });
    ...
    const ws = XLSX.utils.json_to_sheet([
      {
        ...
        city: "Mumbai",
        state: "Maharashtra",
        productInterest: "ISI Mark Certification",
        notes: "Interested in 50L loan",
      },
    ]);
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      showToast(
        `Imported ${res.data.imported} leads. Duplicates: ${res.data.duplicates}`,
      );
      setModal(null);
      setImportRows([]);
      setImportPreview([]);
  ...
  const columns = [
    ...
    {
      label: "Address",
      render: (lead) => lead.state || "—",
    },
    {
      label: "Services",
      render: (lead) => lead.productInterest || "—",
    },
    {
      label: "Status",
      render: (lead) => (
    ...
    {
      label: "Assigned To",
      render: (lead) => lead.assignedToName || "—",
    },
    {
      label: "Actions",
      render: (lead) =>
    ...
    <div className="flex flex-col flex-1 min-w-0 overflow-hidden p-6 h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Manage and assign leads to CRM users
          </p>
        </div>
      ...
      {/* <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: "Unallocated", value: stats.unallocated, color: "text-amber-600" },
          { label: "Allocated", value: stats.allocated, color: "text-emerald-600" },
        ].map((s) => (
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        {[
          ...
          // { key: "productInterest", label: "Product", options: PRODUCTS },
        ...
        <button
          onClick={fetchLeads}
          className="p-2 border border-gray-200 rounded-lg bg-white hover:bg-gray-50 text-gray-500 hover:text-gray-700 transition-colors cursor-pointer"
        >
          <RefreshCw size={15} />
        </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-gray-100">
            ...
            <h2 className="text-lg font-bold text-gray-900 text-center mb-1">
              ...
              {deleteTarget.ids.length > 1
                ? `${deleteTarget.ids.length} Leads`
                : "Lead"}
              ?
            </h2>
            <p className="text-sm text-gray-500 text-center mb-6">
              {deleteTarget.ids.length > 1
                ? `These ${deleteTarget.ids.length} leads will be permanently removed. This action cannot be undone.`
                : "This lead will be permanently removed. This action cannot be undone."}
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteTarget(null)}
                className="flex-1 py-2.5 border border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 text-sm font-medium cursor-pointer transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={doDelete}
                disabled={deleting}
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-50 rounded-xl text-white text-sm font-medium cursor-pointer transition-colors"
              >
            ...
            <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
              {users.map((u) => (
                ...
                >
                  ...
                  {assignUser?.id === (u.id || u.uid) && (
                    <Check size={16} className="ml-auto text-gray-900" />
                  )}
                </button>
              ))}
            </div>
...
{[
  ...
  <div key={key}> {/* Fixed */}
    ...
    <input
      type="text"
      placeholder="e.g., ISI Mark, BIS Certification, Custom Service"
    ...
    />
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 transition-all resize-none"
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-gray-900 hover:bg-gray-700 rounded-xl text-white text-sm font-medium cursor-pointer"
                >
                  Add Lead
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-xl flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Use this template for correct column format
                </p>
              </div>
              <button
                onClick={downloadTemplate}
                className="flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-100 border border-gray-200 rounded-lg text-sm text-gray-700 font-medium cursor-pointer"
              >
            ...
            <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center mb-4 hover:border-gray-300 transition-colors">
              ...
              <label
                htmlFor="xlfile"
                className="cursor-pointer px-4 py-2 bg-gray-900 hover:bg-gray-700 rounded-lg text-sm text-white font-medium transition-colors"
              >
                Choose File
              </label>
            </div>
            ...
            {importRows.length > 0 && (
              <div className="mb-4">
                <p className="text-sm text-gray-700 font-medium mb-2">
                  Preview — {importRows.length} rows found, showing first 5
                </p>
                    ...
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-100">
                        {[
                          ...
                          <th
                            key={h}
                            className="px-3 py-2 text-gray-500 text-left font-semibold uppercase tracking-wider"
                          >
                            {h}
                          </th>
                        ))}
