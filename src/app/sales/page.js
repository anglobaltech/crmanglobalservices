    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm font-semibold text-gray-800">Lead Details</p>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-700 cursor-pointer"
          >
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div className="min-w-0">
                ...
                <a
                  ...
                  className="text-sm font-semibold text-gray-800 hover:text-emerald-600 break-all"
                  onClick={(e) => e.stopPropagation()}
                >
                  {lead.email}
                </a>
          ...
          {lead.productInterest && (
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div>
                <p className="text-xs text-gray-400 mb-0.5">Product Interest</p>
                <p className="text-sm font-semibold text-gray-800">
                  {lead.productInterest}
                </p>
              </div>
                    ...
                    {lead.quotationShared ? (
                      <span className="text-emerald-600">
                        ...
                        {lead.quotationAmount
                          ? ` · ₹${Number(lead.quotationAmount).toLocaleString("en-IN")}`
                          : ""}
                      </span>
                    ) : (
...
function LeadHistoryPanel({ leadId }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get("/api/activity", { params: { leadId } })
  ...
  if (!logs.length)
    return (
      <div className="py-8 text-center text-gray-400 text-xs">
        <History size={22} className="mx-auto text-gray-200 mb-2" />
        No history yet
      </div>
    );
  return (
      ...
      <div className="space-y-4">
        {logs.map((log) => {
          const ns = log.newData?.status,
            os = log.oldData?.status,
            sc = STATUS_CONFIG[ns] || {};
          return (
            <div key={log.id} className="flex gap-3 relative">
              <div
                className={`w-2.5 h-2.5 rounded-full border-2 border-white flex-shrink-0 mt-1 z-10 ${sc.dot || "bg-gray-300"}`}
              ...
              <div className="flex-1">
                ...
                {os && ns && (
                  <div className="flex items-center gap-1 mb-1 flex-wrap">
                    <span
                      className={`px-1.5 py-0.5 rounded-full text-xs border ${STATUS_CONFIG[os]?.color || "bg-gray-100 text-gray-500 border-gray-200"}`}
                    >
                      {STATUS_CONFIG[os]?.label || os}
                    </span>
                    <ChevronRight size={10} className="text-gray-400" />
                    <span
                      className={`px-1.5 py-0.5 rounded-full text-xs border font-medium ${sc.color || "bg-gray-100 text-gray-500 border-gray-200"}`}
                    >
                      {sc.label || ns}
                    </span>
                  </div>
                ...
                {log.newData?.meetingSubType && (
                  <p className="text-xs text-cyan-600 mb-0.5">
                    {log.newData.meetingSubType === "online"
                      ? "Online"
                      : log.newData.meetingSubType === "offline"
                        ? "Offline"
                        : "Reschedule"}
                  </p>
                )}
                {log.newData?.followupDate && (
                  <p className="text-xs text-blue-600 mb-0.5">
                    Next Follow-up: {fmtDate(log.newData.followupDate)}
                  </p>
                )}
                {log.newData?.meetingDate && (
                  <p className="text-xs text-cyan-600 mb-0.5">
                    {fmtDateTime(log.newData.meetingDate)}
                  </p>
                )}
...
function FollowupPopup({ leads, onUpdate, onDismiss }) {
  ...
  if (!lead) return null;
  const handleNext = () => {
    if (idx < leads.length - 1) setIdx(idx + 1);
    else onDismiss();
  };
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-amber-200 overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          ...
          {lead.followupDate && (
            <div
              className={`rounded-xl px-3 py-2 border ${lead.followupDate < todayStr() ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}
            >
              <p
                className={`text-xs font-medium ${lead.followupDate < todayStr() ? "text-red-700" : "text-amber-700"}`}
              >
                {lead.followupDate < todayStr()
                  ? `⚠ Overdue: ${fmtDate(lead.followupDate)}`
                  : "Follow-up: Today"}
              </p>
            </div>
          )}
          ...
          <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${STATUS_CONFIG[lead.status]?.color || "bg-gray-100 text-gray-600 border-gray-200"}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${STATUS_CONFIG[lead.status]?.dot || "bg-gray-400"}`}
            />
        ...
        <div className="px-5 pb-5 flex gap-2">
          <button
            ...
            className="flex-1 py-2.5 bg-gray-900 hover:bg-gray-700 text-white rounded-xl text-sm font-medium cursor-pointer"
          >
            Update Now
          </button>
          {leads.length > 1 ? (
            <button
              onClick={handleNext}
              className="flex-1 py-2.5 border border-gray-200 text-gray-600 hover:bg-gray-50 rounded-xl text-sm font-medium cursor-pointer"
            >
              Next ({leads.length - idx - 1} left)
            </button>
          ) : (
            <button
              onClick={onDismiss}
...
export default function SalesPage() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      if (isQuotationTab)
        fl = fl.filter((lead) => lead.quotationShared === true);
  ...
  const openUpdate = (lead) => {
    ...
    setUpdateForm({
      ...
      followupDate: "",
      followUpNote: "",
      meetingSubType: lead.meetingSubType || "",
      meetingDate: "",
      meetingNote: "",
  ...
  const openUpdateFromPopup = (lead) => {
  ...
  const submitUpdate = async () => {
    ...
    try {
      await api.patch(`/api/sales/leads/${selectedLead.id}/status`, {
        ...updateForm,
        quotationAmount:
          updateForm.quotationShared === true ? updateForm.quotationAmount : "",
      });
    ...
    },
    {
      ...
      render: (lead) => (
        <button
          ...
          className="px-2.5 py-1.5 bg-gray-900 hover:bg-gray-700 text-white rounded-lg text-xs font-medium cursor-pointer whitespace-nowrap"
        >
          Update
        </button>
      ),
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      {showFollowup && (
        <FollowupPopup
          leads={followupLeads}
          onUpdate={openUpdateFromPopup}
          onDismiss={() => setShowFollowup(false)}
        />
      )}
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg text-sm font-medium border ${toast.type === "error" ? "bg-red-50 text-red-700 border-red-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}
        >
          {toast.type === "error" ? (
            <AlertCircle size={16} />
          ) : (
        ...
        <div className="flex items-center gap-2">
          {followupLeads.length > 0 && !showFollowup && (
            <button
              ...
              className="relative p-2 bg-amber-50 border border-amber-200 rounded-lg text-amber-600 hover:bg-amber-100 cursor-pointer"
            >
              <Bell size={16} />
          ...
          <button
            onClick={fetchLeads}
            className="p-2 border border-gray-200 rounded-lg bg-white hover:bg-gray-50 text-gray-500 cursor-pointer shadow-sm"
          >
            <RefreshCw size={15} />
          </button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl border border-gray-100 max-h-[92vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
              ...
              <button
                ...
                className="text-gray-400 hover:text-gray-600 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 border-r border-gray-100">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {ALL_STATUSES.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full flex-shrink-0 ${active ? "bg-white" : sc.dot}`}
                          />
                          <span>{sc.label}</span>
                        </button>
                      );
                ...
                {isMeeting && (
                  <div className="p-4 bg-cyan-50 border border-cyan-200 rounded-xl space-y-3">
                    <label className="text-xs font-semibold text-cyan-700 block">
                      Meeting Type & Details
                    </label>
                    <div className="flex gap-2">
                      {[
                        { val: "online", label: "Online", cls: "cyan" },
                        { val: "offline", label: "Offline", cls: "cyan" },
                        {
                          val: "reschedule",
                          label: "Reschedule",
                          cls: "orange",
                    ...
                    <input
                      type="text"
                      placeholder={
                        updateForm.meetingSubType === "reschedule"
                          ? "Reason for reschedule..."
                          : "Meeting Conversation / note..."
                      }
                      value={updateForm.meetingNote}
                      onChange={(e) =>
                        setUpdateForm({
                          ...updateForm,
                ...
                <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl space-y-3">
                  <label className="text-xs font-semibold text-indigo-700 block flex items-center gap-1.5">
                    <FileText size={13} />
                    Quotation Shared?
                  </label>
                  <div className="flex gap-2">
                  ...
                  {updateForm.quotationShared === true && (
                    <div className="relative">
                      ...
                      <input
                        ...
                        value={updateForm.quotationAmount}
                        onChange={(e) =>
                          setUpdateForm({
                            ...updateForm,
                ...
                <div className="bg-gray-50 rounded-xl p-3 text-xs text-gray-500 space-y-1 border border-gray-100">
                  ...
                  {selectedLead.productInterest && (
                    <p> Service Interest: {selectedLead.productInterest}</p>
                  )}
                  {selectedLead.state && (
                    <p>
                      {" "}
                      Location: {selectedLead.state}
                  ...
                  {selectedLead.createdAt && (
                    <p> Created At: {fmtDate(selectedLead.createdAt)}</p>
                  )}
                  {selectedLead.followupDate && (
                    <p>
                      {" "}
                      Previous follow-up: {fmtDate(selectedLead.followupDate)}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="px-6 pb-5 pt-3 border-t border-gray-100 flex gap-3 flex-shrink-0">
              <button
                onClick={() => setModal(null)}
                className="flex-1 py-2.5 border border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 text-sm font-medium cursor-pointer"
              >
                Cancel
              </button>
              <button
